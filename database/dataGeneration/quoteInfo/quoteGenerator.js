...
const generateQuotes = (startIndex, endIndex, cb) => {
  ...
  for (let i = startIndex; i < endIndex; i += 1) {
    let data = [];
    data.push(prices.generateDailyPrices(basePrice, tickerGenerator(i)));
    data.push(prices.generateWeeklyPrices(basePrice, tickerGenerator(i)));
    data.push(prices.generateMonthlyPrices(basePrice, tickerGenerator(i)));
    data.push(prices.generatePricesPerPeriod(basePrice, tickerGenerator(i), 90, 'Quarterly'));
    data.push(prices.generatePricesPerPeriod(basePrice, tickerGenerator(i), 365, 'Annually'));
    data.push(prices.generatePricesPerPeriod(basePrice, tickerGenerator(i), 365 * 5, 'Quinquennial'));
    data = data.join('');
    if (i === endIndex - 1) {
      cb(data, () => {
